...
void cf::WindowCoordinateSystem3D::drawPlane(const glm::vec4& vec, const cf::Color& color, uint8_t alpha) {
    ...
    for (int idx0 = 0; idx0 < 3; ++idx0) {
        for (int idx1 = 0; idx1 < 3; ++idx1) {
            ...
            for (const auto& e :
                 {std::make_pair(min, min), std::make_pair(min, max), std::make_pair(max, min), std::make_pair(max, max)}) {
                int calcIdx = -1;
                for (int i = 0; i < 3; ++i) {
                    if (i != idx0 && i != idx1) {
                        calcIdx = i;
...
void cf::WindowCoordinateSystem3D::drawSphere(const glm::vec4& point, const cf::Color& color, float size) {
    std::lock_guard<std::mutex> guard(this->m_WriteMutex);
...
void cf::WindowCoordinateSystem3D::draw() {
    ...
    for (const auto& e : this->m_Points)
        cf::Window3D::drawSphere(e.pos, e.size, e.color); // has to be explicit call to window3D
