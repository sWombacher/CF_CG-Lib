...
void Window2D::setWindowDisplayScale(float scale) { this->m_WindowScale = scale; }
float Window2D::getWindowDisplayScale() const { return this->m_WindowScale; }

void Window2D::setInvertYAxis(bool invert) { this->m_InvertYAxis = invert; }
bool Window2D::getInvertYAxis() const { return this->m_InvertYAxis; }

void Window2D::drawCirclePart(cf::Point center, int radius, float startAngle, float endAngle, int lineWidth,
                              const cf::Color& c) {
    ...
    cv::ellipse(this->m_Image, center, cv::Size(radius, radius), 0.f, startAngle, endAngle, cv::Scalar(c.b, c.g, c.r),
                lineWidth);
}
...
void Window2D::drawLine(const Line& line) { this->drawLine(line.point1, line.point2, line.lineWidth, line.color); }
void Window2D::drawRectangle(const Rect& rect) { this->drawRectangle(rect.point1, rect.point2, rect.lineWidth, rect.color); }
void Window2D::drawCircle(const Circle& circle) {
    this->drawCircle(circle.center, circle.radius, circle.lineWidth, circle.color);
}
void Window2D::drawCirclePart(const CirclePartition& cp) {
    this->drawCirclePart(cp.center, cp.radius, cp.startAngle, cp.endAngle, cp.lineWidth, cp.color);
}
    ...
    for (int i = 0; i < iter.count; ++i, ++iter) {
        if (!(i % freq)) {
            ...
            if (type & Window2D::DASH_VALUE) {
                // draw dash
                for (int lengthCounter = 0; lengthCounter < 2 * freq && i < iter.count; ++lengthCounter, ++iter, ++i) {
...
void Window2D::setNewInterval(const Interval& intervalX, const Interval& intervalY) {
    ...
    if (intervalX.min != 0.f || intervalX.max != this->m_Image.cols - 1 || intervalY.min != 0.f ||
        intervalY.max != this->m_Image.rows - 1) {
...
void Window2D::saveImage(const char* filePath) const { cv::imwrite(filePath, this->m_Image); }

void Window2D::resize(int pixelWidth, int pixelHeight) { cv::resize(this->m_Image, this->m_Image, {pixelWidth, pixelHeight}); }

void Window2D::flippHorizontal() { cv::flip(this->m_Image, this->m_Image, 0); }
void Window2D::flippVertical() { cv::flip(this->m_Image, this->m_Image, 1); }

int Window2D::getWidth() const { return this->m_Image.cols; }
int Window2D::getHeight() const { return this->m_Image.rows; }
cv::Mat& Window2D::getImage() { return this->m_Image; }
...
void Window2D::drawAxis(const cf::Color& color, float stepSize_x, float stepSize_y, float interceptLength) {
    ...
    auto drawAxisInterceptions = [&](bool horizontalLine) {
        ...
        for (float start = startPos * stepSize; start <= interval.max; start += stepSize, ++iterationCounter) {
            // draw vertical line
...
const cf::Interval& Window2D::getIntervalX() const { return this->m_IntervalX; }
const cf::Interval& Window2D::getIntervalY() const { return this->m_IntervalY; }
...
Point Point::operator*(float factor) const { return {this->x * factor, this->y * factor}; }
...
Point Point::operator/(float rhs) const { return {this->x / rhs, this->y / rhs}; }
...
Point::operator cv::Point() const { return cv::Point(std::round(this->x), std::round(this->y)); }
